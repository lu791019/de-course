...
- [第八部分：hahow-crawler 整合版 B](#第八部分hahow-crawler-整合版-b)
...
## 第三部分：de-course 真實專案 Dockerfile

用課程的 de-course repo（ep03-04）打包真實的爬蟲專案。這個 Dockerfile 用 **uv** 管理環境。
...
hahow-crawler 是**真實的資料工程專案**，比 ep03-04 多了 **worker**（執行爬蟲）和 **producer**（發送任務）。有三種操作方式（分開版 / 整合版 A / 整合版 B），以下前置步驟三種都要先做。
...
| phpMyAdmin 一直 Restarting（Apple Silicon）| 改用 `phpmyadmin:latest`（多架構），hahow-crawler 已修好 |
| worker `Failed to spawn: 'celery'` | command 已改成 `uv run python -m celery`（hahow-crawler 已修好）|
| producer `ModuleNotFoundError: data_ingestion` | environment 要有 `PYTHONPATH=/app`（hahow-crawler 已加）|
| producer `Connection refused` exit(1) | RabbitMQ 還沒 ready，等 20-30 秒再跑 `docker compose up producer` |
| worker 爬完但 MySQL `Unknown database` | compose 的 `MYSQL_DATABASE` 要和程式碼一致（hahow-crawler 已改成 `hahow`）|
| worker 連不到 MySQL `127.0.0.1 refused` | compose 的 worker environment 要有 `MYSQL_HOST=mysql`（hahow-crawler 已加）|
